...
function generateCollectionFiles(baseDir, collectionFilePaths, options) {
  return Promise.all(
    collectionFilePaths.map(filePath =>
      renderCollectionFile(baseDir, filePath, options).then(contents =>
  ...
  return new Promise((resolve, reject) => {
    glob(`${dirPath}/+(*.yml|*.yaml)`, (err, files) => {
      ...
      const contents = files
        ...
        .reduce((s, file) => {
          const refPath = options.relativePaths ? file.base : path.relative(baseDir, file.filePath)
          if (isMap) {
